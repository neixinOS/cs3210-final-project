...
env_alloc(struct Env **newenv_store, envid_t parent_id)
{
  ...
  // Clear the divide by zero handler until user installs one.
  e->env_divide0_upcall = 0;
  // Clear the general protection fault handler until user installs one.
  e->env_gpfault_upcall = 0;
  // Clear the illegal opcode handler until user installs one.
  e->env_illegalop_upcall = 0;
...
region_alloc(struct Env *e, void *va, size_t len)
{
  ...
  struct PageInfo *pg;
    ...
    if(!pg) {
	  ...
	  page_insert(e->env_pgdir, pg, begin, PTE_U | PTE_W);
  }
}
...
load_icode(struct Env *e, uint8_t *binary)
{
  ...
  if (elfhdr->e_magic != ELF_MAGIC) {  
    panic("elfhdr magic is not correct\n");
  }
