...
trap_init(void)
{
  ...
  SETGATE(idt[6], 0, GD_KT, illegal_op, 0);
  ...
  //undefined 15
  SETGATE(idt[16], 0, GD_KT, float_point_error, 0);
  SETGATE(idt[17], 0, GD_KT, alignment_check, 0);
  SETGATE(idt[18], 0, GD_KT, machine_check, 0);
  SETGATE(idt[19], 0, GD_KT, SIMD_float_point_error, 0);*/
  int i;
  for (i = 0; i < 20; i++) {
    // allow users to make breakpoint exceptions
    if (i == T_BRKPT) {
      SETGATE(idt[i], 0, GD_KT, table[i], 3);
    } else if (i != 9 && i != 15) {
      SETGATE(idt[i], 0, GD_KT, table[i], 0);
    }
  }
  ...
  // irqs
  for (i = 0; i < 16; i++) {
    SETGATE(idt[IRQ_OFFSET + i], 0, GD_KT, table[20 + i], 0);
  }
  // Per-CPU setup
  trap_init_percpu();
}
...
general_protection_fault_handler(struct Trapframe * tf)
{
  ...
  if ((tf->tf_cs & 3) == 0) {
    panic("a general protection fault happens in user mode");
  }
  ...
  cprintf("[%08x] user general protection fault va %08x ip %08x\n",
    curenv->env_id, fault_va, tf->tf_eip);
  print_trapframe(tf);
  env_destroy(curenv);
...
illegal_opcode_handler(struct Trapframe * tf)
{
  ...
  if ((tf->tf_cs & 3) == 0) {
    panic("an illegal opcode fault happens in user mode");
  }
  if (curenv->env_illegalop_upcall != NULL) {
    struct UTrapframe *utf;
    uintptr_t addr;
    if ((tf->tf_esp >= (UXSTACKTOP - PGSIZE)) && (tf->tf_esp < UXSTACKTOP)) {
      addr = tf->tf_esp - sizeof(struct UTrapframe) - 4;
    ...
    curenv->env_tf.tf_eip = (uintptr_t)curenv->env_illegalop_upcall;
    curenv->env_tf.tf_esp = addr;
    env_run(curenv);
  }

  cprintf("[%08x] user illegal opcode va %08x ip %08x\n",
    curenv->env_id, fault_va, tf->tf_eip);
  print_trapframe(tf);
  env_destroy(curenv);
}
