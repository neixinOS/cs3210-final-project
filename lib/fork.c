...
pgfault(struct UTrapframe *utf)
{
  ...
  if ((err & FEC_WR) == 0) {
    panic("page fault was not caused by write!");
  }
  if ((uvpt[PGNUM(addr)] & PTE_COW) == 0) {
    panic("page fault was not to a COW page");
  }
  ...
  r = sys_page_map(0, PFTEMP, 0, (void *)ROUNDDOWN(addr, PGSIZE), PTE_P|PTE_U|PTE_W);
  if (r) {
...
fork(void)
{
  ...
  /*r = sys_page_map(envid, (void *)(UXSTACKTOP - PGSIZE), 0, PTE_P|PTE_U|PTE_W);
  if (r) {
    return r;
  }
